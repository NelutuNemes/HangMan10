...
// Definesc clasa 'ThemeManager' care se va ocupa de gestionarea temelor
class ThemeManager {
    // Constructorul clasei
    constructor() {
      // Obține elementele HTML relevante utilizând metoda document.getElementById
      this.themeSelect = document.getElementById("theme-select");
      this.submitButton = document.getElementById("submit-button2");
      this.bodyTheme = document.getElementById("body");
      this.keyboardTheme = document.getElementById("keyboard");
      this.keyboardName1 = document.getElementById("keyboard-name1");
      this.footerContainer = document.getElementById("footer-container");
      this.currentTheme = "Clean-white"; // Tema curentă implicită
  
      // Adaugă un eveniment "click" pentru butonul de schimbare a temei și leagă funcția changeTheme de instanța curentă a clasei
      this.submitButton.addEventListener("click", this.changeTheme.bind(this));
    }
  
    // Metodă pentru schimbarea temei
    changeTheme(event) {
      // Oprirea comportamentului implicit al butonului de submit (evitarea reîncărcării paginii)
      event.preventDefault();
  
      // Obține tema selectată din elementul select
      const selectedTheme = this.themeSelect.value;
  
      // Verifică dacă tema selectată este diferită de cea curentă
      if (selectedTheme !== this.currentTheme) {
        // Elimină clasele CSS ale temei anterioare, dacă există
        this.bodyTheme.classList.remove(
          "gradient-background",
          "gradient-background2",
          "gradient-background3"
        );
        this.keyboardTheme.classList.remove(
          "gradient-background",
          "gradient-background2",
          "gradient-background3"
        );
        ...
        // Actualizează culoarea de fundal a întregii pagini în funcție de tema selectată
        switch (selectedTheme) {
          case "Clean-white":
            // Șterge proprietatea CSS pentru culoarea de fundal
            document.documentElement.style.removeProperty("--main-background-color");
            // Setează culoarea de fundal a corpului și culoarea textului
            this.bodyTheme.style.backgroundColor = "#ffffff";
            this.keyboardName1.style.color = "#000000";
            break;
          case "Barbie-Pink":
            // Setează culoarea de fundal pentru tema Barbie-Pink
            document.documentElement.style.setProperty("--main-background-color", "#fffff");
            // Adaugă clasele CSS pentru gradientul de fundal
            this.bodyTheme.classList.add("gradient-background");
            this.keyboardTheme.classList.add("gradient-background");
            break;
          case "Blue-blade":
            // Setează culoarea de fundal pentru tema Blue-blade
            document.documentElement.style.setProperty("--main-background-color", "#fffff");
            // Adaugă clasele CSS pentru gradientul de fundal
            this.bodyTheme.classList.add("gradient-background2");
            this.keyboardTheme.classList.add("gradient-background2");
            // Setează culoarea textului
            this.keyboardName1.style.color = "#ffffff";
            this.footerContainer.style.color = "#ffffff";
            break;
          case "Green-beetle":
            // Setează culoarea de fundal pentru tema Green-beetle
            document.documentElement.style.setProperty("--main-background-color", "#ffffff");
            // Adaugă clasele CSS pentru gradientul de fundal
            this.bodyTheme.classList.add("gradient-background3");
            this.keyboardTheme.classList.add("gradient-background3");
            // Setează culoarea textului
            this.keyboardName1.style.color = "#000000";
            break;
          default:
  ...
  // Crearea unei instanțe a clasei ThemeManager pentru a gestiona temele
